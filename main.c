...
int editorRowRxtoCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t') {
      cur_cx += (KILO_TAB_STOP - 1) - (cur_cx % KILO_TAB_STOP);
    }
    cur_cx++;
    if (cur_cx > rx) {
      return cx;
...
void editorRowDelChar(erow *row, int at) {
  ...
  memcpy(&row->chars[at], &row->chars[at+1], row->size - at);
  row->size--;
  editorUpdateRow(row);
  E.dirty++;
}
...
void editorFindCallback(char *query, int key) {
  ...
  static int saved_line_hl;
  static int *saved_hl = NULL;

  if (saved_hl) {
    memcpy(E.row[saved_line_hl].hl, saved_hl, E.row[saved_line_hl].rsize);
    free(saved_hl);
    saved_hl = NULL;
  }
  if (key == '\r' || key == '\x1b') {
...
void editorFind() {
  ...
  char *query = editorPrompt("Search : %s (ESC to quit | Arrows to jump) ", editorFindCallback);
...
void editorRefreshScreen() {
  ...
  abAppend(&ab, "\x1b[H", 3); //put cursor on first row and first column
  ...
  char buf[32];
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH", (E.cy - E.rowoff) + 1, (E.rx - E.coloff) + 1); //moving cursor to its current position
